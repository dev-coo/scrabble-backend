...
**이 파일은 백엔드 쪽 참고용입니다. 복사해 가면 그 순간부터 낡습니다.**
...
`/docs` 에서는 **버튼을 눌러 바로 호출해 볼 수도 있습니다.** (Try it out)

> 서버가 켜져 있어야 보입니다. 안 열리면 백엔드 담당에게 서버를 켜달라고 하세요.

> 📌 **예전에는 이 파일을 프론트엔드 저장소에도 복사해 두고 함께 고쳤습니다.**
> 그러면 한쪽만 고치는 날 두 문서가 서로 다른 말을 하고, 어느 쪽이 맞는지
> 아무도 모르게 됩니다. 이제 볼 곳을 **주소 하나**로 모읍니다.
...
| GET | `/api/game/setup` | 아래 참고 | **칩 구성 · 보드 배열** (게임 고정 규칙) |

> 새 API를 만들면 **여기에 추가**하고 프론트엔드 담당에게 알려주세요.
>
> `POST /api/players` 와 `GET /api/players` 는 **주소가 같고 메서드만 다릅니다.**
> 같은 대상(players)에 추가하면 POST, 조회하면 GET — REST 규칙입니다.

> ### ⚠️ 이 표가 이 백엔드의 전부가 아닙니다
>
> 방 만들기 · 코드로 입장 · 랜덤 매칭 · 1:1 채팅 · **게임 시작**은 전부
> **웹소켓**이라 이 표에도, 스와거(`/docs`)에도 나오지 않습니다.
> 그쪽은 [`docs/ws-contract.md`](./ws-contract.md) 와
> <http://localhost:11000/ws-docs> 를 보세요.
>
> **프론트엔드 담당은 두 문서를 다 봐야 합니다.** 한쪽만 보면 절반을 놓칩니다.
...
### GET `/api/game/setup` — 칩 구성 · 보드 배열

게임을 그리는 데 필요한 **고정 값**입니다. 칩(타일) 100개 구성과 15×15 보드 배열.

> 📌 **언제 불러도 항상 같은 값입니다.** 방마다 달라지지 않습니다.
...
| 필드 | 설명 |
|------|------|
| `board_size` | 보드 한 변의 칸 수 (15) |
| `board` | 보드 배열. **위에서 아래로** 한 줄씩, `board[줄][칸]` 으로 찾습니다 (둘 다 0부터) |
| `premium_legend` | 특수 칸 기호의 뜻 |
| `center` | 한가운데 시작 칸 `[줄, 칸]`. **첫 단어는 반드시 여기를 지나야 합니다** |
| `tiles` | 칩 구성 27종 |
| `total_tiles` | 가방에 든 전체 개수 (100) |
| `rack_size` | 한 사람이 손에 드는 수 (7) |

#### 보드 읽는 법

빈 문자열 `""` 은 **보통 칸**입니다. 나머지는 `premium_legend` 에 뜻이 있습니다.

- `applies_to: "letter"` → 그 칸에 놓인 **글자 하나**에만 배수가 걸립니다
- `applies_to: "word"` → **단어 전체** 점수에 배수가 걸립니다

특수 칸 개수: 단어3배 8칸 · 단어2배 16칸 · 글자3배 12칸 · 글자2배 24칸 · 시작 1칸.
보드는 **90도 돌려도 같은 모양**이라, 한 칸이라도 다르면 그림이 어긋나 보입니다.

#### 칩 구성

| 점수 | 글자 |
|------|------|
| 1점 | E×12 A×9 I×9 O×8 N×6 R×6 T×6 L×4 S×4 U×4 |
| 2점 | D×4 G×3 |
| 3점 | B×2 C×2 M×2 P×2 |
| 4점 | F×2 H×2 V×2 W×2 Y×2 |
| 5점 | K×1 |
| 8점 | J×1 X×1 |
| 10점 | Q×1 Z×1 |
| 0점 | **`?`×2 (빈 타일)** |

> 📌 **`"?"` 는 빈 타일입니다.** 아무 글자로나 쓸 수 있는 대신 **0점**입니다.
> 알파벳으로 표시하면 진짜 그 글자와 구별이 안 돼서 `?` 를 씁니다.

> 📌 흔한 글자는 많이·싸게, 귀한 글자는 적게·비싸게 되어 있습니다.
> (E 는 12개 1점, Q 는 1개 10점) 이게 스크래블의 균형입니다.

> ⚠️ **이 API 에는 "지금 이 게임의 상태"가 없습니다.**
> 누가 어떤 칩을 들고 있는지, 보드에 무엇이 놓였는지는 방마다 다르고 매 순간
> 바뀌는 값이라 **웹소켓**으로 오갑니다. → [`docs/ws-contract.md`](./ws-contract.md)
>
> **아직 칩을 나눠주는 기능은 없습니다.** 지금은 "무엇이 있는가"까지입니다.
...
- **웹소켓(WebSocket)은 이 문서에 없습니다.** 스와거가 자동 문서화해 주지 않으므로
  ...
  현재 상태: 방 만들기 · 코드로 입장 · 랜덤 매칭 · 1:1 채팅(DB 저장) ·
  **게임 시작**까지. 게임 로직(타일·판·차례·점수)은 아직 없습니다.
- **스와거(`/docs`) 첫 화면에도 위 안내가 떠 있습니다.** 스와거만 열어본
  사람이 "플레이어 API 다섯 개가 전부"라고 오해하지 않게 하려는 것입니다.
  스와거가 웹소켓 경로 자체를 그려줄 수는 없어서, **어디로 가야 하는지**를
  대신 적어 두었습니다.
